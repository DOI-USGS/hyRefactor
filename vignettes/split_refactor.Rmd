...
nhdplusTools::plot_nhdplus(outlets = new_hope_flowline$COMID, 
                           nhdplus_data = file.path(extdata, "new_hope.gpkg"), 
                           flowline_only = TRUE)
...
```{r}
split <- hyRefactor::split_flowlines(suppressWarnings(
  sf::st_cast(sf::st_transform(new_hope_flowline, 5070), "LINESTRING")), 
  max_length = 2000, events = new_hope_events)

split_outlets <- sf::st_sf(sf::st_drop_geometry(split), 
                           geom = sf::st_geometry(nhdplusTools::get_node(split, position = "end")))
